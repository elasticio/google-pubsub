...
async function processAction(msg, cfg) {
  ...
  async function synchronousPull() {
    ...
    const formattedTopic = subClient.topicPath({
      project: projectId,
      topic: topicName,
    });
    ...
    const isExists = await subClient.getSubscription(request);
    if (!isExists[0].name) {
      // create a subscription
      const createdSubsc = await subClient.createSubscription(request);
      self.logger.info(createdSubsc);
    }
    ...
    for (const message of response.receivedMessages) {
      self.logger.info(`Received message: ${message.message.data}`);
      msgs.push(message);
